...
impl ExecutionStatuses {
    ...
    ///
    /// # Parameters
    /// - `txn_idx`: The transaction being stalled
    /// - `by_tx`: The upstream transaction causing this stall
    ///
    /// # Returns
    ...
    pub(crate) fn add_stall(&self, txn_idx: TxnIndex, by_tx: TxnIndex) -> Result<bool, PanicError> {
        self.add_stall_internal(txn_idx, by_tx)
    }

    /// Legacy add_stall without by_tx parameter (for tests)
    #[cfg(test)]
    pub(crate) fn add_stall_legacy(&self, txn_idx: TxnIndex) -> Result<bool, PanicError> {
        self.add_stall_internal(txn_idx, txn_idx)
    }

    fn add_stall_internal(&self, txn_idx: TxnIndex, by_tx: TxnIndex) -> Result<bool, PanicError> {
        let status = &self.statuses[txn_idx as usize];
        let prev_stall_count = status.num_stalls.fetch_add(1, Ordering::SeqCst);
        let current_stall_count = prev_stall_count + 1;
        let first_stall = prev_stall_count == 0;
        
        // Log StallAdd event for every stall, recording the actual upstream transaction
        if let Some(logger) = get_global_logger() {
            ...
            logger.log_stall_add(txn_idx, incarnation, by_tx, current_stall_count, first_stall);
    ...
    pub(crate) fn remove_stall(&self, txn_idx: TxnIndex, owner_txn: TxnIndex) -> Result<bool, PanicError> {
        ...
        let current_stall_count = prev_num_stalls.saturating_sub(1);
        let became_unstalled = prev_num_stalls == 1;
        ...
        // Log StallRemove event
        if let Some(logger) = get_global_logger() {
            let status_guard = status.status_with_incarnation.lock();
            let incarnation = status_guard.incarnation();
            logger.log_stall_remove(txn_idx, incarnation, owner_txn, current_stall_count, became_unstalled);
            drop(status_guard); // Release early to avoid holding lock too long
        }
...
mod tests {
    ...
    fn status_cycle_with_finish_and_resolve(stall_before_finish: bool) {
        ...
        if stall_before_finish {
            assert_ok_eq!(statuses.add_stall_legacy(txn_idx), true);
        }
    ...
    fn stall_executed_status() {
        ...
        assert_ok_eq!(statuses.add_stall_legacy(0), true);
        assert_eq!(
        ...
        assert_ok_eq!(statuses.add_stall_legacy(0), false);
        assert_ok_eq!(statuses.add_stall_legacy(0), false);
        assert_ok_eq!(statuses.add_stall_legacy(0), false);
    ...
    fn add_remove_stall_simple_scheduling() {
        ...
        assert_err!(statuses.add_stall_legacy(0));
        assert_err!(statuses.add_stall_legacy(1));
        ...
        assert_ok_eq!(statuses.add_stall_legacy(2), true);
        manager.assert_execution_queue(&vec![]);
    }
    ...
    fn stall_pending_scheduling(incarnation: Incarnation, txn_idx: TxnIndex) {
        ...
        *statuses.get_status_mut(txn_idx) = ExecutionStatus::new_for_test(
        ...
        assert_eq!(status.num_stalls.load(Ordering::Relaxed), 1);
        assert_ok_eq!(statuses.add_stall_legacy(txn_idx), false);
        assert_ok_eq!(statuses.add_stall_legacy(txn_idx), false);
        assert_eq!(status.num_stalls.load(Ordering::Relaxed), 3);
        assert_eq!(
            status.dependency_shortcut.load(Ordering::Relaxed),
