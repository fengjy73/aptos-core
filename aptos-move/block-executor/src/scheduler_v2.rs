    ...
    pub(crate) fn next_task(&self) -> Result<TaskKind, PanicError> {
        ...
        match self.pop_post_commit_task()? {
            Some(txn_idx) => {
                ...
                if let Some(logger) = get_global_logger() {
                    logger.log_task_picked_v2(
                        "PostCommit",
                        txn_idx,
                        0, // PostCommit tasks don't have incarnation
                        false, // PostCommit is always after execution
                        "PostCommitQueue",
                    );
                }
        ...
        if let Some(txn_idx) = self.txn_statuses.get_execution_queue_manager().pop_next() {
            if let Some(incarnation) = self.start_executing(txn_idx)? {
                // Log TaskPickedV2 event with enhanced V2 tracking
                if let Some(logger) = get_global_logger() {
                    logger.log_task_picked_v2(
                        "Execute",
                        txn_idx,
                        incarnation,
                        incarnation == 0, // is_first_execution
                        "ExecutionQueue",
                    );
                }
    ...
    pub(crate) fn finish_execution<'a>(
        ...
        for (dep_txn_idx, maybe_incarnation) in invalidated_set {
            if let Some(incarnation) = maybe_incarnation {
                ...
                let _finish_result = self.txn_statuses.finish_abort(dep_txn_idx, incarnation, true)?;
                
                // Log AbortFinish event
                if let Some(logger) = get_global_logger() {
                    let result = "EnqueuedForReexec"; // Since add_to_schedule is true
                    let new_incarnation = Some(incarnation + 1);
                    logger.log_abort_finish(
                        dep_txn_idx,
                        incarnation,
                        txn_idx, // The transaction that caused this abort
                        result,
                        new_incarnation,
                    );
                }
        ...
        if self.txn_statuses.finish_execution(txn_idx, incarnation)? {
            // Log TaskFinishedV2 event with enhanced V2 tracking
            if let Some(logger) = get_global_logger() {
                ...
                // Determine defer_reason based on execution context
                let defer_reason = if is_reexecution {
                    Some("Reexecution".to_string())
                } else if txn_idx > executed_once_watermark {
                    Some("Waterline".to_string())
                } else {
                    None // First execution, no deferral
                };
...
impl SchedulerV2 {
    ...
    fn start_abort(&self, txn_idx: TxnIndex, incarnation: Incarnation) -> Result<bool, PanicError> {
        ...
        // Log AbortStart event
        if let Some(logger) = get_global_logger() {
            let result = if ret { "Started" } else { "AlreadyAborted" };
            logger.log_abort_start(
                txn_idx, 
                incarnation, 
                txn_idx, // Use self as by_tx for now - will be overridden by caller context if needed
                result
            );
        }
    ...
    fn try_increase_executed_once_max_idx(&self, txn_idx: TxnIndex) {
        ...
        let previous_waterline = execution_queue_manager
            .executed_once_max_idx
            .load(Ordering::Relaxed);
            
        if previous_waterline == txn_idx {
            let mut idx = txn_idx;
            while idx < self.num_txns && self.txn_statuses.ever_executed(idx) {
                ...
                let new_waterline = idx + 1;
                execution_queue_manager
                    .executed_once_max_idx
                    .store(new_waterline, Ordering::Relaxed);
                    
                // Log waterline advance when it actually changes
                if let Some(logger) = get_global_logger() {
                    logger.log_waterline_advance(
                        new_waterline,
                        txn_idx, // The transaction that triggered the advance
                        previous_waterline,
                    );
                }
...
mod tests {
    ...
    fn propagate() {
        ...
        assert_ok_eq!(scheduler.txn_statuses.add_stall_legacy(4), true);
    ...
    fn stall_and_add_dependency(
        ...
        assert_ok_eq!(scheduler.txn_statuses.add_stall_legacy(dep_idx), true);
        assert!(scheduler.txn_statuses.get_status(dep_idx).is_stalled());
        for _ in 1..num_stalls {
            assert_ok_eq!(scheduler.txn_statuses.add_stall_legacy(dep_idx), false);
        }
        assert!(scheduler.txn_statuses.get_status(dep_idx).is_stalled());
