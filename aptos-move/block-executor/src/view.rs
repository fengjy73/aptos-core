        ...
        loop {
            ...
            match group_size {
                ...
                Err(Dependency(dep_idx)) => {
                    if !wait_for_dependency(&self.scheduler, txn_idx, self.incarnation, dep_idx)? {
        ...
        loop {
            let data = if self.scheduler.is_v2() {
                self.versioned_map.data().fetch_data_and_record_dependency(
                    ...
                    txn_idx,
                    self.incarnation,
                    self.versioned_map.logger().map(|l| l.as_ref()),
                )
            } else {
                self.versioned_map.data().fetch_data_no_record(key, txn_idx, self.versioned_map.logger().map(|l| l.as_ref()))
            };
            ...
            match data {
                ...
                Err(Dependency(dep_idx)) => {
                    match wait_for_dependency(&self.scheduler, txn_idx, self.incarnation, dep_idx) {
                        Err(e) => {
