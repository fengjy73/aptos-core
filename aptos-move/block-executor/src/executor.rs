    ...
    TP: TxnProvider<T> + Sync,
{
                    ...
                    Some((group_key, (group_metadata_op, group_size, group_ops))) => {
                        ...
                        abort_manager.invalidate_dependencies(
                            ...
                            versioned_cache.data().write_v2::<true>(
                                ...
                                None,
                                versioned_cache.logger().map(|l| l.as_ref()),
                            ),
                        )?;
                        abort_manager.invalidate_dependencies(
    ...
    fn execute_v2(
        ...
        if let Some(logger) = get_global_logger() {
            logger.start_transaction_execution(idx_to_execute, incarnation);
        ...
        if let Some(output) = maybe_output {
            ...
            for (key, value, maybe_layout) in resource_write_set.clone().into_iter() {
                ...
                let invalidated_deps = versioned_cache.data().write_v2::<false>(
                    key.clone(),
                    idx_to_execute,
                    incarnation,
                    value,
        ...
        for key in prev_modified_resource_keys {
            ...
            // Log invalidation edges for dependency tracking (resource removals)
            if let Some(logger) = get_global_logger() {
                for (dep_txn, dep_incarnation) in &invalidated_deps {
                    logger.log_invalidation_edge(
                        idx_to_execute,
        ...
        // Extract basic data for logging before recording execution_result
        let (gas_used, write_set_size) = if let Some(output) = maybe_output {
            (output.fee_statement().gas_used(), resource_write_set.len())
        } else {
            (0, 0)
        };
        ...
        if let Some(logger) = get_global_logger() {
            ...
            // Get accurate read/write statistics from execution output and read set
            // For reads: use actual_read_set_size as total reads
            // For writes: extract precise counts from execution result
            let (precise_resource_writes, precise_module_writes, precise_delayed_field_writes, 
                 precise_aggregator_writes) = if let Some(output_ref) = last_input_output.txn_output(idx_to_execute) {
                match output_ref.as_ref() {
                    ExecutionStatus::Success(detailed_output) | ExecutionStatus::SkipRest(detailed_output) => {
                        (
                            resource_write_set.len(), // Accurate resource writes
                            detailed_output.module_write_set().len(), // Accurate module writes
                            detailed_output.delayed_field_change_set().len(), // Accurate delayed field writes
                            detailed_output.aggregator_v1_write_set().len(), // Accurate aggregator writes
                        )
                    },
                    _ => (0, 0, 0, 0)
                }
            ...
            // For execution_flow.ndjson, use accurate data
            let resource_reads = actual_read_set_size; // Total reads (accurate)
            let resource_writes = precise_resource_writes + precise_aggregator_writes; // Total resource writes
            let module_reads = 0; // Cannot separate easily, included in total reads above
            let module_writes = precise_module_writes; // Accurate module writes
            let delayed_field_reads = 0; // Cannot separate easily, included in total reads above  
            let delayed_field_writes = precise_delayed_field_writes; // Accurate delayed field writes
            
            // Record transaction execution statistics
            let is_reexecution = incarnation > 1;
            logger.record_transaction_execution(is_reexecution);
            logger.finish_transaction_execution(idx_to_execute, incarnation);
            ...
            // Log dependency resolution when transaction completes successfully
            // This simulates dependencies being resolved by this transaction's completion
            if execution_result_str == "Success" || execution_result_str == "SkipRest" {
                let num_txns = signature_verified_block.num_txns();
                // Log dependency resolution for some potential dependents
            ...
            // Log read/write set changes (simplified version)
            if let Some(read_set_ref) = last_input_output.read_set(idx_to_execute) {
                ...
                // Log detailed read/write statistics to specialized log file
                // Use the accurate statistics we calculated above
                logger.log_readwrite_detailed_stats(
                    idx_to_execute,
                    incarnation,
                    actual_read_set_size, // Total resource reads (accurate)
                    precise_resource_writes, // Accurate resource writes only
                    0, // module_reads (cannot separate accurately, included in total above)
                    precise_module_writes, // Accurate module writes
                    0, // delayed_field_reads (cannot separate accurately, included in total above)
                    precise_delayed_field_writes, // Accurate delayed field writes
                    0, // aggregator_v1_reads (cannot separate accurately, included in total above)
                    precise_aggregator_writes, // Accurate aggregator writes
                    actual_read_set_size, // Total read summary size
                    precise_resource_writes + precise_module_writes + precise_delayed_field_writes + precise_aggregator_writes, // Total write summary size
                );
            }
    ...
    fn execute(
        ...
        > {
            ...
            for (k, v, maybe_layout) in resource_write_set.clone().into_iter().chain(
                ...
                versioned_cache
                    .data()
                    .write(k, idx_to_execute, incarnation, v, maybe_layout, versioned_cache.logger().map(|l| l.as_ref()));
            }
        ...
        loop {
            ...
            match scheduler.next_task()? {
                ...
                TaskKind::PostCommitProcessing(txn_idx) => {
                    if let Some(logger) = crate::block_stm_logger::get_global_logger() {
                        // Record post-commit task statistics
                        logger.record_task_type("PostCommit");
                        logger.increment_post_commit_tasks();
                    }
                ...
                TaskKind::NextTask => {
                    ...
                    // This is normal system idle time, not a Block-STM stall
    ...
    pub(crate) fn execute_transactions_parallel_v2(
        ...
        if let Some(logger) = get_global_logger() {
            ...
            logger.log_block_finish_legacy(
                &format!("block_{}", std::process::id()),
