...
impl<V: TransactionWrite + PartialEq> VersionedValue<V> {
    ...
    fn read<StateKey: std::fmt::Debug>(
        &self,
        ...
        while let Some((idx, entry)) = iter.next_back() {
            ...
            match (&entry.value, accumulator.as_mut()) {
                (
                    ...
                    // Log MV read operation
                    if let Some(logger) = logger {
                        let writer_tx = idx.idx().ok();
                        let state_key = format!("{:?}", key);
                        let read_from = "MVCommitted";
                        let value_size = Some(std::mem::size_of_val(&value_with_layout));
        ...
        // Log MV read operation when no entry is found (reading from storage)
        if let Some(logger) = logger {
            let state_key = format!("{:?}", key);
            let read_from = match &accumulator {
                Some(Ok(_)) => "Unresolved",
                Some(Err(_)) => "DeltaFailure", 
                None => "Storage"
            };
...
impl<K: Hash + Clone + Debug + Eq, V: TransactionWrite + PartialEq> VersionedData<K, V> {
    ...
    where
        Q: Equivalent<K> + Hash + std::fmt::Debug,
    {
        ...
        Q: Equivalent<K> + Hash + std::fmt::Debug,
    {
        self.values
            .get(key)
            .map(|v| v.read(txn_idx, None, logger, key))
        ...
        Q: Equivalent<K> + Hash + std::fmt::Debug,
    {
        self.values
            .get(key)
            .map(|v| v.read(txn_idx, Some(incarnation), logger, key))
            .unwrap_or(Err(MVDataError::Uninitialized))
    }
    ...
    fn write_impl<StateKey: std::fmt::Debug>(
        ...
        // Log MV write operation
        if let Some(logger) = logger {
            let state_key = format!("{:?}", key);
            let value_size = std::mem::size_of_val(&value);
            let write_type = "Modify";
    ...
    pub fn materialize_delta(&self, key: &K, txn_idx: TxnIndex) -> Result<u128, DeltaOp> {
        ...
        match v.read(txn_idx + 1, None, None, key) {
...
mod tests {
    ...
    fn test_split_off_affected_read_dependencies(
        ...
        let _ = v.read(3, Some(0), None, &test_key); // This adds (3, 0) to latest write <= 3 (write at idx 0).
        let _ = v.read(3, Some(1), None, &test_key); // Add another incarnation of txn 3.
        let _ = v.read(7, Some(1), None, &test_key); // This adds (7, 1) to write at idx 0.
        let _ = v.read(8, Some(1), None, &test_key); // This adds (8, 1) to write at idx 7 (duplicate with existing).
        let _ = v.read(10, Some(0), None, &test_key); // Add lower incarnation after we'll add a higher one.
        let _ = v.read(10, Some(2), None, &test_key); // Add higher incarnation first.
