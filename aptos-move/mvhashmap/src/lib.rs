...
/// Trait for logging MV operations - allows dependency injection without circular dependencies
pub trait MVLogger: Send + Sync {
    fn log_mv_read(
        &self,
        txn_id: types::TxnIndex,
        incarnation: types::Incarnation,
        state_key: &str,
        read_from: &str,
        writer_tx: Option<types::TxnIndex>,
        writer_incarnation: Option<types::Incarnation>,
        is_estimate: bool,
    ...
    I: Copy + Clone + Eq + Hash + Debug,
{
    ...
    /// Get logger reference for use in operations
    pub fn logger(&self) -> Option<&Arc<dyn MVLogger>> {
        self.logger.as_ref()
    }
